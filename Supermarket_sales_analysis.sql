...
Select Item_Identifier , min(Item_Weight) AS Min_weight 
from big_mart
Group by Item_Identifier;

# 5) WRITE a query to show average Item_Weight.
Select Item_Identifier , avg(Item_Weight) AS Avg_Item_Weight
from big_mart
Group by Item_identifier;
...
SELECT Outlet_Establishment_Year, COUNT(Item_Identifier) AS No_of_items
FROM big_mart
